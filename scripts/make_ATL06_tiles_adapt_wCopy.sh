...
for file in $cycle_dir/*ATL06*.h5; do
    ...
    echo $file
    echo "${script_path}/pointCollection/pointCollection/index_glob.py -t ATL06 -H $hemisphere --index_file $this_file_index -g $file --dir_root `pwd`/$dir/" >> file_queue.txt
done
...
${script_path}/pointCollection/pointCollection/index_glob.py --dir_root=`pwd`/$cycle_dir/index/ -t h5_geoindex -H $hemisphere --index_file $cycle_dir/index/GeoIndex.h5 -g "$cycle_dir/index/*ATL06*.h5" -v --Relative
...
${script_path}/pointCollection/scripts/make_tiles.py -H $hemisphere -i $cycle_dir/index/GeoIndex.h5 -W 100000 -t ATL06 -o $cycle_tile_dir -q tile_queue.txt -j ${script_path}/ATL11/ATL06_field_dict.json
# run the queue
...
${script_path}/pointCollection/pointCollection/index_glob.py -H $hemisphere -t indexed_h5 --index_file GeoIndex.h5 -g "E*.h5" --dir_root `pwd` -v 
popd
