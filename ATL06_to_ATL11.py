...
from ATL11_misc import ATL11_defaults, default_ATL06_fields
...
def fit_ATL11(ATL06_files, beam_pair=1, ref_pt_numbers=None, output_file=None, num_ref_pts=None, first_ref_pt=None, DOPLOT=None, DEBUG=None, mission_time_bds=None, verbose=False):
    ...
    ATL06_field_dict=default_ATL06_fields()
    D6=ATL06_data(beam_pair=beam_pair).from_list([ATL06_data(field_dict=ATL06_field_dict).from_file(filename) for filename in ATL06_files])
...
def main():
    ...
    ATL06_files=glob(args.ATL06_glob.replace('"',''))
    if args.verbose:
        print("working on :")
        print(ATL06_files)
